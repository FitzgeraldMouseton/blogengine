...
public interface VoteRepository extends CrudRepository<Vote, Integer> {
    ...
    @Query("select count(v) from Vote v join v.post p where p.active = true and p.user = :user and v.value = :value")
    long countVotesOfUserPosts(User user, byte value);

    @Query("select count(v) from Vote v join v.post p where p.active = true and v.value = :value")
